...
import PlayoffResultsSkeleton from '@/features/matches/components/PlayoffResultsSkeleton';
...
function PlayoffResultsInner({ seasonId, title, initialTab }: Props) {
  const PAGE_SIZE = 6;
            ...
            <div>
              ...
              {isFetchingNextPage && (
                <div className="mt-6 flex items-center justify-center">
                  <span
                    className="inline-block h-6 w-6 animate-spin rounded-full border-2 border-gray-300 border-t-purple-600"
...
export default function PlayoffResults({ seasonId, title }: Props) {
  return (
    <GoalWrapper fallback={<PlayoffResultsSkeleton />}>
      <PlayoffResultsInner seasonId={seasonId} title={title} />
    </GoalWrapper>
  );
}
