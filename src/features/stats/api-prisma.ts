...
export const getPlayerSeasonStatsPrisma = async (
  ...
  const response = await fetch(
    apiUrl(`/api/stats/player-season?season_id=${seasonId}`)
  );
  if (!response.ok) {
    throw new Error(
...
export const getPlayerSeasonStatsByPlayerPrisma = async (
  ...
  const response = await fetch(
    apiUrl(`/api/stats/player-season?player_id=${playerId}`)
  );
  if (!response.ok) {
    throw new Error(
      `Failed to fetch player season statistics: ${response.statusText}`
...
export const getStandingsPrisma = async (
  ...
  const response = await fetch(apiUrl(`/api/seasons/${seasonId}/standing`));
  if (!response.ok) {
